...
static String buildJWT(time_t now) {
  ...
  mbedtls_md(
    ...
    (const uint8_t*)sigInput.c_str(),
    sigInput.length(),
    hash
  );

  // Sign using mbedtls 2.x API (ESP32 Arduino core)
...
// Fetch events from one calendar ID into events[] starting at offset
...
int gcal_getEvents(CalEvent* events, int maxEvents) {
  ...
  // Sort by date/time string (simple lexicographic — works because format is consistent)
  // For now just return in calendar order
...
int gcal_getTasks(CalEvent* events, int maxEvents) {
  ...
  // Only fetch incomplete tasks due within 7 days
  const char* url =
  ...
  http.end();
  Serial.printf("GCal: tasks response length = %d\n", respBody.length());
  Serial.println(respBody);  // print full response — it's only 72 bytes

  JsonDocument doc;
  DeserializationError err = deserializeJson(doc, respBody);
  if (err) {
    Serial.printf("GCal: tasks JSON error: %s\n", err.c_str());
    return 0;
  }
  ...
  for (JsonObject item : doc["items"].as<JsonArray>()) {
    ...
    // Tasks have "due" field (date only, no time)
